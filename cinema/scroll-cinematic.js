...
function initScrub(cfg) {
  ...
  const isLive  = typeof cfg.render === "function"; /* live mode: cfg.render(ctx, canvas, p) */
  const images = [];
  let firstDrawn = false;
  if (!isLive) for (let i = 0; i < cfg.frameCount; i++) {
    const img = new Image();
  ...
  let lastP = 0;
  function draw(index) {
    if (isLive) { cfg.render(ctx, canvas, lastP); return; }
